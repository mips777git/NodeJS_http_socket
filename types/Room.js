...
class Room {
  ...
  join(client, clientId = null) {
    ...
    // 새 클라이언트 접속 로그 기록
    SocketLogger.info(
      `새 클라이언트 접속 [ID: ${id}] (총 ${this.clients.size}명)`
    );
  ...
  leave(clientOrId) {
    ...
    if (removed) {
      // 제거 성공 시 로그 기록
      SocketLogger.info(
        `클라이언트 퇴장 [ID: ${clientId}] (남은 클라이언트: ${this.clients.size}명)`
      );
    } else {
      // 제거 실패 시 (이미 없는 클라이언트) 경고 로그
      SocketLogger.warn(
        `제거하려는 클라이언트 [ID: ${clientId}]가 방에 존재하지 않습니다`
      );
    }
  ...
  forwardMessage(message) {
    // 메시지 브로드캐스트 시작 로그
    SocketLogger.info(
      `메시지 브로드캐스트 시작 (대상: ${this.clients.size}명)`
    );
    ...
    for (const [clientId, clientInfo] of this.clients) {
      try {
        ...
        if (client.readyState === 1) {
          ...
          // 연결이 열려있지 않은 클라이언트
          SocketLogger.warn(
            `[ID: ${clientId}] 연결이 닫힌 클라이언트에게 메시지 전송 실패`
          );
          failCount++;
        }
      } catch (error) {
        // 전송 중 에러 발생 시 로그 기록
        SocketLogger.error(
          `[ID: ${clientId}] 메시지 전송 실패: ${error.message}`
        );
        failCount++;
      }
    }
    ...
    // 브로드캐스트 완료 로그
    SocketLogger.info(
      `메시지 브로드캐스트 완료 (성공: ${successCount}, 실패: ${failCount})`
    );
  }
